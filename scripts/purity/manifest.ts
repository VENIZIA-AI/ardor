...
export interface IPurityEntry {
   ...
   * Never for ARDOR's own source: an external hides that file from the module graph entirely, so
    ...
    for (const specifier of row.external ?? []) {
      ...
      throw new Error(
        `[purity][manifest] Entry '${row.label}' externalises '${specifier}', a workspace package ` +
          `| \`external\` may only exempt THIRD-PARTY packaging, never ARDOR's own code - doing so ` +
