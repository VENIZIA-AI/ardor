 ...
 * every `.md` file under the given directories and extracts three kinds of reference: an ARDOR
 * `blob/main/<path>` or `tree/main/<path>` GitHub link target - scoped to the `VENIZIA-AI/ardor`
...
const GITHUB_LINK_PATTERN = /VENIZIA-AI\/ardor\/(?:blob|tree)\/main\/([^\s)\]"'`<>]+)/g;
