#!/usr/bin/env bun
/**
 * okf.ts - generator + conformance gate for the ARDOR OKF knowledge bundle.
 *
...
type ClassEntry = { dir: string; classes: string[] };

const relDir = (dir: string): string => relative(REPO, dir);

const collectProviders = (): ClassEntry[] => {
  return PATHS.providerDirs
    .filter((dir) => existsSync(dir))
    .map((dir) => ({ dir: relDir(dir), classes: exportedClasses({ dir, suffix: 'Provider' }) }));
};

const collectServices = (): ClassEntry[] => {
  return PATHS.serviceDirs
    .filter((dir) => existsSync(dir))
    .map((dir) => ({ dir: relDir(dir), classes: exportedClasses({ dir, suffix: 'Service' }) }));
};

type HookEntry = { dir: string; hooks: string[] };

/** Every exported `use*` arrow or function in a hooks directory - the row vocabulary of the catalog. */
const collectHooks = (): HookEntry[] => {
  return PATHS.hookDirs
    .filter((dir) => existsSync(dir))
    .map((dir) => {
      const names = new Set<string>();
      for (const file of sourceFiles({ dir })) {
        for (const match of readFileSync(file, 'utf8').matchAll(/export\s+(?:const|function)\s+(use[A-Z]\w*)/g)) {
          names.add(match[1]);
        }
      }
      return { dir: relDir(dir), hooks: [...names].sort() };
    });
};
 ...
 * brand "Ardor". The bundle is documentation, so it follows the docs rules (hyphen, always
 * "ARDOR") no matter where the text came from - and we do not rewrite package.json, which is
 * published to npm.
 */
...
const renderProviders = (): GeneratedFile => {
  ...
  return {
    path: join(BUNDLE, 'reference/providers.md'),
    content:
      refHeader({
        title: 'Providers catalog',
        description: 'Every provider class the framework ships, by package directory (generated).',
        resource: 'packages/admin/src/providers',
        tags: 'reference, providers, catalog',
      }) +
      generatedNote({ seeAlso: 'Model: [data provider pipeline](/architecture/data-provider-pipeline.md).' }) +
      `**${total} provider classes across ${entries.length} directories.**\n\n` +
      ['| Directory | Classes |', '|---|---|', ...rows].join('\n') +
      '\n',
  };
};

const renderHooks = (): GeneratedFile => {
  const entries = collectHooks();
  const services = collectServices();
  const total = entries.reduce((sum, entry) => sum + entry.hooks.length, 0);
  const rows = entries.map((entry) => {
    const hooks = entry.hooks.map((name) => `\`${name}\``).join(' · ') || '_(see source)_';
    return `| \`${entry.dir}/\` | ${hooks} |`;
  });
  const serviceRows = services.map((entry) => {
    const classes = entry.classes.map((name) => `\`${name}\``).join(' · ') || '_(see source)_';
    return `| \`${entry.dir}/\` | ${classes} |`;
  });

  return {
    path: join(BUNDLE, 'reference/hooks-and-services.md'),
    content:
      refHeader({
        title: 'Hooks and services catalog',
        description: 'Every exported React hook and every service class, by package directory (generated).',
        resource: 'packages/react/src/hooks',
        tags: 'reference, hooks, services, catalog',
      }) +
      generatedNote({ seeAlso: 'Model: [hooks and context](/architecture/hooks-and-context.md).' }) +
      `**${total} hooks across ${entries.length} directories.**\n\n` +
      ['| Directory | Hooks |', '|---|---|', ...rows].join('\n') +
      '\n\n## Services\n\n' +
      ['| Directory | Classes |', '|---|---|', ...serviceRows].join('\n') +
      '\n',
  };
};
...
const RENDERERS: { id: string; render: () => GeneratedFile }[] = [
  ...
  { id: 'providers', render: renderProviders },
  { id: 'hooks-and-services', render: renderHooks },
...
const check = (): void => {
  ...
  // 3. docs style - hyphen not dash, brand always ARDOR.
...
const coverage = (): void => {
  ...
  ref({
    name: 'providers',
    count: providers.reduce((sum, entry) => sum + entry.classes.length, 0),
    generated: hasId({ id: '/reference/providers' }),
  });
  ref({
    name: 'hooks',
    count: hooks.reduce((sum, entry) => sum + entry.hooks.length, 0),
    generated: hasId({ id: '/reference/hooks-and-services' }),
  });
  ref({
    name: 'binding keys',
